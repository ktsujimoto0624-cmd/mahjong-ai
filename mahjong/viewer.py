...
/* 河エリア: 6牌ずつ並べる */
.river-area { display: grid; gap: 1px; }
...
.river-left {
    ...
    align-content: center; min-width: 36px;
    overflow: hidden;
    direction: rtl;
}
...
function renderHand(seat, isVertical) {
    let html = '';
    // South (seat 1): reverse order for 90° CW rotation
    let reversed = (seat === 1);
...
function renderRiver(seat, isVertical) {
    ...
    // South (seat 1): reverse within each group of 6 for 90° CW rotation
    if (seat === 1) {
        tiles = [];
        for (let i = 0; i < rivers[seat].length; i += 6) {
            let group = rivers[seat].slice(i, Math.min(i + 6, rivers[seat].length));
            tiles.push(...group.reverse());
        }
    }
    let html = '';
...
function render() {
    ...
    let winner = (RESULT && (RESULT.type === "tsumo" || RESULT.type === "ron")) ? RESULT.winner : -1;

    // seat 0=東(bottom), 1=南(right), 2=西(top), 3=北(left)
    let positions = [
    ...
    for (let p of positions) {
        ...
        let rMark = riichiState[p.seat] ? '<span class="riichi-mark">リーチ</span>' : '';
        let label = '<div class="hand-label"><span>' + SEAT_NAMES[p.seat] + rMark +
                    '</span><span class="agent">' + AGENTS[p.seat] + '</span></div>';
        let tiles = '<div class="hand-tiles">' + renderHand(p.seat, p.vertical) + '</div>';
    ...
    document.getElementById("center-info").innerHTML =
        "<div style='font-size:18px;'>巡目: " + turn + "</div>" +
        "<div style='font-size:12px;color:#8a8;margin-top:4px;'>残り: " +
    ...
    if (step >= 0) {
        let a = ACTIONS[step];
        if (a.type === "riichi") {
            info = SEAT_NAMES[a.seat] + " リーチ!";
        } else {
            info = SEAT_NAMES[a.seat] + " " +
                   (a.type === "draw" ? "ツモ " : "打 ") + TILE_NAMES[a.tile];
        }
    }
    ...
    if (isEnd) {
        ...
        if (RESULT.type === "tsumo") {
            banner.innerHTML = "ツモ和了! " + SEAT_NAMES[RESULT.winner] +
                " (" + AGENTS[RESULT.winner] + ") " + RESULT.turn + "巡目 和了牌: " +
                '<img class="tile-img" src="' + tileSvgUrl(RESULT.winning_tile) +
                '" style="vertical-align:middle;">';
        } else if (RESULT.type === "ron") {
            banner.innerHTML = "ロン和了! " + SEAT_NAMES[RESULT.winner] +
