...
/* 河エリア */
.river-area { display: flex; flex-wrap: wrap; gap: 1px; }
.river-top { justify-content: center; min-height: 40px; }
...
function renderRiver(seat, isVertical) {
    let html = '';
    for (let r = 0; r < rivers[seat].length; r++) {
        let cls = rivers[seat][r].isLast ? "discard-last" : "";
        if (rivers[seat][r].riichi) cls += " riichi-tile";
        html += isVertical ? tileRiverVert(rivers[seat][r].tile, cls) :
                             tileRiver(rivers[seat][r].tile, cls);
    }
    return html;
}
