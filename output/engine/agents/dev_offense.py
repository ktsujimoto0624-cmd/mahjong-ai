"""
攻撃型エージェント (DevAgentベース)

打点重視・ドラ温存・積極的リーチ。
- BETAORI_SHANTEN = 1（テンパイ以外は守備しにくい）
- RIICHI_MIN_WAITS = 1（テンパイ即リーチ）
- DANGER_WEIGHT = 0.5（攻撃重視）
- DORA_KEEP_BONUS = 5（ドラをより残す）
- CALL_YAKUHAI_AGGRESSIVELY = True
- CALL_SHANTEN_THRESHOLD = False（門前重視で打点を上げる）
"""
...
class OffenseAgent(AgentBase):
    """攻撃型エージェント。打点重視・ドラ温存・積極的リーチ。"""

    model = "offense-v1"
    description = (
        "攻撃型。打点重視・ドラ温存・積極的リーチ。"
    )
