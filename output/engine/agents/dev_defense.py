"""
守備型エージェント (DevAgentベース)

ベタオリ重視・安全牌優先・慎重なリーチ判断。
- BETAORI_SHANTEN = 3（早めに守備）
- RIICHI_MIN_WAITS = 3（待ちが広い時だけリーチ）
- DANGER_WEIGHT = 1.5（危険牌をより避ける）
- DORA_KEEP_BONUS = 3（標準）
- CALL_YAKUHAI_AGGRESSIVELY = True
- CALL_SHANTEN_THRESHOLD = False（鳴きは控えめ）
"""
...
class DefenseAgent(AgentBase):
    """守備型エージェント。ベタオリ重視・安全牌優先・慎重なリーチ判断。"""

    model = "defense-v1"
    description = (
        "守備型。ベタオリ重視・安全牌優先・慎重なリーチ判断。"
    )
