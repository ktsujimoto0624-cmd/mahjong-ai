"""
鳴き重視型エージェント (DevAgentベース)

副露で素早くテンパイ。速攻型。
- BETAORI_SHANTEN = 2（標準）
- RIICHI_MIN_WAITS = 1（標準）
- DANGER_WEIGHT = 1.0（標準）
- DORA_KEEP_BONUS = 2（効率重視）
- CALL_YAKUHAI_AGGRESSIVELY = True
- CALL_SHANTEN_THRESHOLD = True（積極的に鳴く）
- choose_pon / choose_chi を緩和: シャンテンが進むなら常に鳴く
"""
...
class NakiAgent(AgentBase):
    """鳴き重視型エージェント。副露で素早くテンパイ。速攻型。"""

    model = "naki-v1"
    description = (
        "鳴き重視型。副露で素早くテンパイ。速攻型。"
    )
    ...
    def choose_chi(self, player, tile, from_seat, game_state):
        """鳴き重視型: シャンテンが進むなら積極的にチー（門前維持チェック緩和）"""
        if not is_suit(tile):
            return None
