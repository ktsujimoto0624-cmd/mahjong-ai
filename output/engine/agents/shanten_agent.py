...
AlphaJongのアルゴリズムを参考に強化:
1. 危険牌の回避（筋・壁分析、放銃率の低減）
2. 攻守バランス（ベタオリ切り替え）
3. 打点を考慮した打牌選択
4. 鳴き判断の改善（手牌価値を考慮）
5. リーチ判断の最適化（待ち・残り枚数・打点を評価）
"""
...
class HiyokoAgent(AgentBase):
    """ひよこエージェント (AlphaJong参考の攻守バランス型)"""

    model = "hiyoko-v1"
    description = (
        "攻守バランス型。危険牌回避・ベタオリ・鳴き判断・打点考慮。"
    )
    ...
    def choose_discard(self, player, game_state):
        ...
        # ベタオリ: リーチ者がいて自分が遠い場合
        if (any_riichi and current_shanten >= 2
    ...
    def _choose_balanced_discard(self, player, game_state, visible,
        ...
        for tile_id in range(NUM_TILE_TYPES):
            ...
            danger_penalty = danger * SAFETY_WEIGHT
    ...
    def choose_pon(self, player, tile, from_seat, game_state):
        ...
        if tile == seat_wind or tile == round_wind:
            return True
